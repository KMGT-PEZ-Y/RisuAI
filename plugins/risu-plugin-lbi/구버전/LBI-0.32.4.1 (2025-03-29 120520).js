...
const PLUGIN_SETTING_DEFINITIONS = {
    // 공통 설정
    ...
    hypa_model: {
        ...
        displayName: "모델",
        type: PLUGIN_SETTING_TYPE.STRING,
        options: { candidates: allSupportedModels },
    },
    hypa_prefill: {
        category: ["감정/하이파 설정"],
...
class Logger {
    ...
    static log(level, message, ...optionalParams) {
        if (this.shouldLog(level)) {
            ...
            console.log(`[${timestamp}][${levelString}][${caller}] ${message}`, ...optionalParams);
        }
    }
...
class Utils {
    ...
    static setArgEx(arg, value) {
        setArg(arg, String(value));
    }
    static async risuFetchEx(pluginRequest, url, arg) {
        ...
        switch (requestType) {
            ...
            case REQUEST_TYPE.MEMORY: {
                const memorySettings = Utils.getMemorySettings();
            ...
            case REQUEST_TYPE.TRANSLATION: {
                const translationSettings = Utils.getTranslationSettings();
                return await risuFetch(url, {
                    ...arg,
                    ...(translationSettings.usePlainFetch
...
class PluginSettingsManager {
    ...
    get(key) {
        ...
        const raw = Utils.getArgEx(`${PLUGIN_NAME}::${key}`);
    ...
    toJSON() {
        return Object.keys(this.definitions).reduce((acc, key) => {
            acc[key] = Utils.getArgEx(`${PLUGIN_NAME}::${key}`);
            return acc;
        }, {});
    }
    ...
    fromJSON(json) {
        Object.entries(json).forEach(([key, value]) => {
            if (this.definitions[key]) {
                Utils.setArgEx(`${PLUGIN_NAME}::${key}`, value);
            }
        });
    }
    generatePluginMetadata() {
        return Object.keys(this.definitions).map((key) => `//@arg ${key} string`);
    }
}
class PluginSettingsUI {
   ...
   `;
    ...
    generateSettingField(key, setting) {
        const value = Utils.getArgEx(`${PLUGIN_NAME}::${key}`);
...
class GithubCopilotTokenManagerUI {
    ...
    static TOKEN_ARG_KEY = `${PLUGIN_NAME}::tools_githubCopilotToken`;
    static showModal() {
        const modal = document.createElement("div");
    ...
    static loadSavedToken() {
        ...
        if (tokenInput) {
            const savedToken = Utils.getArgEx(GithubCopilotTokenManagerUI.TOKEN_ARG_KEY);
            if (savedToken) {
    ...
    static bindEvents(modal) {
        ...
        tokenInput?.addEventListener("input", () => {
            Utils.setArgEx(GithubCopilotTokenManagerUI.TOKEN_ARG_KEY, tokenInput.value.trim());
        });
            ...
            return new Promise((resolve, reject) => {
                confirmButton?.addEventListener("click", async () => {
                    try {
                        ...
                        Utils.setArgEx(GithubCopilotTokenManagerUI.TOKEN_ARG_KEY, accessToken);
                        const tokenInput = document.getElementById(GithubCopilotTokenManagerUI.TOKEN_INPUT_ID);
                        if (tokenInput) {
    ...
    static async showStatus() {
        const token = Utils.getArgEx(GithubCopilotTokenManagerUI.TOKEN_ARG_KEY);
        if (!token) {
    ...
    static async autoConfig() {
        ...
        const confirmed = await Utils.confirmEx("다음 설정으로 플러그인의 커스텀 프로바이더를 설정하시겠습니까?\n\n" +
            ...
            "커스텀 플래그: hasFirstSystemPrompt, requiresAlternateRole, mustStartWithUserInput");
        if (!confirmed) {
            return;
        }
        try {
            Utils.setArgEx(`${PLUGIN_NAME}::common_openaiCompatibleProvider_url`, "https://api.githubcopilot.com/chat/completions");
            Utils.setArgEx(`${PLUGIN_NAME}::common_openaiCompatibleProvider_apiKey`, token);
            Utils.setArgEx(`${PLUGIN_NAME}::common_openaiCompatibleProvider_model`, "claude-3.7-sonnet");
            Utils.setArgEx(`${PLUGIN_NAME}::common_openaiCompatibleProvider_hasFirstSystemPrompt`, "1");
            Utils.setArgEx(`${PLUGIN_NAME}::common_openaiCompatibleProvider_requiresAlternateRole`, "1");
            Utils.setArgEx(`${PLUGIN_NAME}::common_openaiCompatibleProvider_mustStartWithUserInput`, "1");
...
class PluginTimerUI {
    ...
    static createGetElement() {
        const chatSettings = Utils.getChatSettings();
...
class RisuCharMessageAutoTranslator {
    ...
    static onKeydown(e) {
        if (e.ctrlKey && e.altKey && e.shiftKey && e.key.toLowerCase() === "t") {
            e.preventDefault();
            const autoClickTranslateButton = Utils.getChatSettings()?.autoClickTranslateButton;
            Utils.setArgEx(`${PLUGIN_NAME}::chat_autoClickTranslateButton`, Number(!autoClickTranslateButton));
...
class RisuTextAreaEnhancer {
    ...
    static initialize() {
        ...
        RisuTextAreaEnhancer.observer = new MutationObserver((mutations) => {
            const commonSettings = Utils.getCommonSettings();
        ...
        switch (requestType) {
            case REQUEST_TYPE.CHAT: {
                ...
                if (chatSettings.claude_useStreaming &&
                    AnthropicProvider.supportedModels.includes(defaultUniqueModel)) {
...
class AutoProvider {
            ...
            try {
                ...
                if (provider instanceof GoogleAIProvider &&
                    ...
                    if (AutoProvider.googleAIProvider.parsedApiKeys.length === 0) {
                        if (commonSettings.fallbackToVertexGemini &&
                            VertexAIProvider.supportedModels.includes(model)) {
    ...
    static getProvider(uniqueModel) {
        const commonSettings = Utils.getCommonSettings();
        // GoogleAIProvider
        if (GoogleAIProvider.supportedModels.includes(uniqueModel)) {
            // Update api key list
        ...
        // VertexAIProvider
        const vertexAIModelMap = new Map(VertexAIProvider.supportedModels.map((e) => [
            e.startsWith("gemini-") ? `vertex-${e}` : e,
            e,
        ]));
        if (vertexAIModelMap.has(uniqueModel)) {
            ...
            const model = vertexAIModelMap.get(uniqueModel);
            return { provider: new VertexAIProvider(credential), model };
        }
        // AnthropicProvider
        if (AnthropicProvider.supportedModels.includes(uniqueModel)) {
            return {
        ...
        // DeepseekProvider
        if (DeepseekProvider.supportedModels.includes(uniqueModel)) {
            return {
        ...
        // OpenAIProvider
        if (OpenAIProvider.supportedModels.includes(uniqueModel)) {
            return {
        ...
        // AWSProvider
        if (AWSProvider.supportedModels.includes(uniqueModel)) {
            return {
...
class GoogleAIProvider extends BaseProvider {
    ...
    static buildGeminiBody(pluginRequest, model) {
        ...
        for (let i = 0; i < openAIChats.length; i++) {
            ...
            if (geminiRole === LLM_ROLES.SYSTEM) {
            ...
            else if (message.role === LLM_ROLES.ASSISTANT &&
                ...
                messages.push({
                    ...
                    parts: [
                        ...
                        ...((Array.isArray(message.thoughts) &&
                            ...
                            trimedContent.toLowerCase() === "{{null}}") ||
                            trimedContent === ""
                            ? []
                            : [
                                {
                ...
                if (lastMessage?.role === geminiRole) {
                    messages[messages.length - 1].parts[0].text += "\n\n" + trimedContent;
                }
                else {
                    messages.push({
    ...
    static isGeminiThinkingModel(model) {
        const thinkingModels = ["gemini-2.5-pro-exp-03-25"];
    ...
    async getResponse(pluginRequest, model) {
        if (!GoogleAIProvider.supportedModels.includes(model)) {
            throw new Error(`Model ${model} not supported by Google AI`);
        }
        ...
        const response = await Utils.risuFetchEx(pluginRequest, url, fetchArgs);
        if (!response.ok) {
            // 429: Quota exceeded
...
class VertexAIProvider extends BaseProvider {
    ...
    static modelLocationsMap = {
        default: ["us-east5", "europe-west1", "asia-southeast1"], // For claude model
        "gemini-1.5-pro-002": ["us-west1"],
        "gemini-1.5-flash-002": ["us-west1"],
        "gemini-2.0-flash-exp": ["us-central1"],
        "gemini-2.0-flash-thinking-exp-01-21": ["us-central1"],
        "gemini-2.0-flash-001": ["us-west1"],
        "gemini-2.0-pro-exp-02-05": ["us-central1"],
        "gemini-2.0-flash-lite-001": ["us-west1"],
        "gemini-2.5-pro-exp-03-25": ["us-central1"],
    };
    ...
    async getResponse(pluginRequest, model) {
        if (!VertexAIProvider.supportedModels.includes(model)) {
            throw new Error(`Model ${model} not supported by Vertex AI`);
        }
        let jsonBody;
        switch (MODEL_FAMILY.identify(model)) {
            case MODEL_FAMILY.CLAUDE: {
        ...
        const availableLocations = VertexAIProvider.modelLocationsMap[model] ||
            VertexAIProvider.modelLocationsMap.default;
        for (let i = 0; i < availableLocations.length; i++) {
            ...
            let response = await Utils.risuFetchEx(pluginRequest, url, fetchArgs);
            ...
            if (!response?.ok && response.data.error?.code === 401) {
                ...
                response = await Utils.risuFetchEx(pluginRequest, url, fetchArgs);
            }
            // Success
            if (response?.ok) {
...
class AnthropicProvider extends BaseProvider {
    ...
    static buildClaudeBody(pluginRequest, model) {
        ...
        if (chatSettings.claude_caching &&
            ...
            if (AnthropicProvider.isExtendedThinking(pluginRequest, model) &&
                ...
                Utils.applyClaudeCaching("s", body);
                TimedOverlay.showMessage("커스텀 브레이크 포인트가 s로 수정됨", 2000);
            }
            else {
                Utils.applyClaudeCaching(chatSettings.claude_cachingBreakpoints, body);
            }
        }
        // Validate api parameters
        AnthropicProvider.validateApiParameters(body);
    ...
    async getResponse(pluginRequest, model) {
        if (!AnthropicProvider.supportedModels.includes(model)) {
            throw new Error(`Model ${model} not supported by Anthropic`);
        }
        const chatSettings = Utils.getChatSettings();
...
class DeepseekProvider extends BaseProvider {
    ...
    async getResponse(pluginRequest, model) {
        if (!DeepseekProvider.supportedModels.includes(model)) {
            throw new Error(`Model ${model} not supported by Deepseek`);
        }
...
class OpenAIProvider extends BaseProvider {
    ...
    async getResponse(pluginRequest, model) {
        if (!OpenAIProvider.supportedModels.includes(model)) {
            throw new Error(`Model ${model} not supported by OpenAI`);
        }
...
class AWSProvider extends BaseProvider {
    ...
    async getResponse(pluginRequest, model) {
        if (!AWSProvider.supportedModels.includes(model)) {
            throw new Error(`Model ${model} not supported by AWS`);
        }
...
class OpenAICompatibleProvider extends BaseProvider {
    ...
    async getResponse(pluginRequest, model) {
        ...
        const response = await Utils.risuFetchEx(pluginRequest, this.url, fetchArgs);
        if (!response.ok) {
            throw new Error(JSON.stringify(response.data));
        }
        const contentPart = response?.data?.choices?.[0]?.message?.content;
...
function addProviderEx(displayName, defaultUniqueModel) {
    /*  const options =
      model === "custom"
        ? Utils.getCommonSettings().openAICompatibleProvider_tokenizer
        : null;
  */
    addProvider(displayName, async (pluginRequest, abortSignal) => {
        try {
            ...
            return Utils.getPluginError(error);
        }
    });
}
const PLUGIN_SETTINGS_MANAGER = new PluginSettingsManager(PLUGIN_SETTING_DEFINITIONS);
allSupportedModels.push("");
// Register providers
allSupportedModels.push("[Google AI]");
GoogleAIProvider.supportedModels.forEach((model) => {
    addProviderEx(`[LBI] [Google AI] ${model}`, model);
    allSupportedModels.push(model);
});
allSupportedModels.push("[Vertex AI]");
VertexAIProvider.supportedModels.forEach((model) => {
    let newModel = model;
    // For Vertex AI, add 'vertex-' prefix to Gemini models to avoid collision
    if (model.includes("gemini")) {
        newModel = `vertex-${model}`;
    }
    addProviderEx(`[LBI] [Vertex AI] ${newModel}`, newModel);
    allSupportedModels.push(newModel);
});
allSupportedModels.push("[Anthropic]");
AnthropicProvider.supportedModels.forEach((model) => {
    addProviderEx(`[LBI] [Anthropic] ${model}`, model);
    allSupportedModels.push(model);
});
allSupportedModels.push("[Deepseek]");
DeepseekProvider.supportedModels.forEach((model) => {
    addProviderEx(`[LBI] [Deepseek] ${model}`, model);
    allSupportedModels.push(model);
});
allSupportedModels.push("[OpenAI]");
OpenAIProvider.supportedModels.forEach((model) => {
    addProviderEx(`[LBI] [OpenAI] ${model}`, model);
    allSupportedModels.push(model);
});
allSupportedModels.push("[AWS]");
AWSProvider.supportedModels.forEach((model) => {
    addProviderEx(`[LBI] [AWS] ${model}`, model);
    allSupportedModels.push(model);
});
allSupportedModels.push("[Custom]");
addProviderEx(`[LBI] [Custom]`, "custom");
allSupportedModels.push("custom");
// Initialize
