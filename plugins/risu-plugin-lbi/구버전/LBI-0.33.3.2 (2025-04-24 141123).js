...
const LLM_DEFINITIONS = [
    ...
    },
    {
        ...
        provider: LLM_PROVIDERS.GOOGLEAI,
        tokenizer: LLM_TOKENIZERS.GEMMA,
        flags: [LLM_FLAGS.isThinkingModel],
    },
    {
    ...
    },
    {
        ...
        provider: LLM_PROVIDERS.GOOGLEAI,
        tokenizer: LLM_TOKENIZERS.GEMMA,
        flags: [LLM_FLAGS.isThinkingModel, LLM_FLAGS.isExperimentalModel],
    },
    {
        ...
        provider: LLM_PROVIDERS.GOOGLEAI,
        tokenizer: LLM_TOKENIZERS.GEMMA,
        flags: [LLM_FLAGS.hasThinkingTokens, LLM_FLAGS.isExperimentalModel],
    },
    // Vertex AI
    {
              ...
              <div class="grid grid-cols-2 gap-4">
                <button id="${PluginToolsUI.EXPORT_SETTINGS_BUTTON_ID}" class="p-4 rounded-lg bg-zinc-700 hover:bg-zinc-500 text-zinc-400 hover:text-zinc-200 transition-colors">
                  <div class="flex flex-col items-center gap-2">
                    ...
                    <span class="text-zinc-200">플러그인 설정 내보내기</span>
                  </div>
                </button>
                ...
                <button id="${PluginToolsUI.IMPORT_SETTINGS_BUTTON_ID}" class="p-4 rounded-lg bg-zinc-700 hover:bg-zinc-500 text-zinc-400 hover:text-zinc-200 transition-colors">
                  <div class="flex flex-col items-center gap-2">
                    ...
                    <span class="text-zinc-200">플러그인 설정 가져오기</span>
                  </div>
                </button>
              </div>
            </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <button id="${PluginToolsUI.EXPORT_INLAY_BUTTON_ID}" class="p-4 rounded-lg bg-zinc-700 hover:bg-zinc-500 text-zinc-400 hover:text-zinc-200 transition-colors">
                  <div class="flex flex-col items-center gap-2">
                    ...
                    <span class="text-zinc-200">인레이 캐시 내보내기</span>
                  </div>
                </button>
                ...
                <button id="${PluginToolsUI.IMPORT_INLAY_BUTTON_ID}" class="p-4 rounded-lg bg-zinc-700 hover:bg-zinc-500 text-zinc-400 hover:text-zinc-200 transition-colors">
                  ...
                  <span class="text-zinc-200">인레이 캐시 가져오기</span>
                  </div>
                </button>
                ...
                <button id="${PluginToolsUI.CLEAR_INLAY_BUTTON_ID}" class="p-4 rounded-lg bg-zinc-700 hover:bg-zinc-500 text-zinc-400 hover:text-zinc-200 transition-colors">
                  <div class="flex flex-col items-center gap-2">
                    ...
                    <span class="text-zinc-200">인레이 캐시 지우기</span>
                  </div>
                </button>
              </div>
            </div>
        ...
        exportInlayButton?.addEventListener("click", async () => {
            try {
                ...
                while (true) {
                    ...
                    const jsonDataArray = new TextEncoder().encode(jsonData);
                    const lengthArray = new Uint32Array([jsonDataArray.length]);
                    await writer.write(new Uint8Array(lengthArray.buffer)); // Write 4-byte length header
                    await writer.write(jsonDataArray); // Write JSON data
                    exportedCount += dataCount;
                    ...
                    while (true) {
                        ...
                        while (buffer.length > 0) {
                            if (readingLength) {
                                if (buffer.length < 4)
                                    break;
                                expectedJsonDataLength = new Uint32Array(buffer.slice(0, 4).buffer)[0]; // Read 4-byte length header
                                buffer = buffer.slice(4);
                                readingLength = false;
                            }
                            else {
                                if (buffer.length < expectedJsonDataLength)
                                    break;
                                const dataArray = buffer.slice(0, expectedJsonDataLength); // Read JSON data
                                buffer = buffer.slice(expectedJsonDataLength);
                                readingLength = true;
                                const jsonData = new TextDecoder().decode(dataArray);
                                const data = JSON.parse(jsonData);
                                for (const [key, value] of Object.entries(data)) {
                        ...
                        PluginProgressUI.updateProgress(Math.min(processedBytes / totalBytes, 1));
                        // Wait to allow UI thread to update
                        await new Promise((resolve) => setTimeout(resolve, 0));
                    }
...
class PluginProgressUI {
    ...
    static progressTextEl;
    static show(message) {
        PluginProgressUI.hide();
        const backdropEl = document.createElement("div");
        backdropEl.className =
            "z-100 fixed inset-0 p-1 sm:p-2 flex items-center justify-center bg-black/70";
        ...
        containerEl.className =
            "flex flex-col p-6 rounded-lg bg-zinc-900 w-96 max-w-full";
        backdropEl.appendChild(containerEl);
        const messageEl = document.createElement("div");
        messageEl.className = "text-lg text-zinc-100 mb-4 text-center";
        messageEl.textContent = message;
        containerEl.appendChild(messageEl);
        const progressContainerEl = document.createElement("div");
        progressContainerEl.className =
            "w-full h-2 rounded-full bg-zinc-700 overflow-hidden mb-2";
        containerEl.appendChild(progressContainerEl);
        const progressBarEl = document.createElement("div");
        progressBarEl.className =
            "h-full bg-blue-500 w-0 transition-all duration-300";
        progressContainerEl.appendChild(progressBarEl);
        const progressTextEl = document.createElement("div");
        progressTextEl.className = "text-sm text-zinc-400 text-center";
        progressTextEl.textContent = "0%";
        containerEl.appendChild(progressTextEl);
        document.body.appendChild(backdropEl);
        PluginProgressUI.backdropEl = backdropEl;
        PluginProgressUI.messageEl = messageEl;
        PluginProgressUI.progressBarEl = progressBarEl;
        PluginProgressUI.progressTextEl = progressTextEl;
    }
    static updateProgress(ratio) {
        if (!PluginProgressUI.progressBarEl || !PluginProgressUI.progressTextEl)
            return;
        const percent = Math.min(Math.round(ratio * 100), 100);
        PluginProgressUI.progressBarEl.style.width = `${percent}%`;
        PluginProgressUI.progressTextEl.textContent = `${percent}%`;
    }
    ...
    static hide() {
        ...
        PluginProgressUI.progressBarEl = null;
        PluginProgressUI.progressTextEl = null;
    }
}
...
class RequestHandler {
    static async handleRequest(pluginRequest, defaultUniqueId) {
        const commonSettings = getCommonSettings();
        const chatSettings = getChatSettings();
...
class GoogleAIProvider extends BaseProvider {
    ...
    async getResponse(pluginRequest, modelDef) {
        const chatSettings = getChatSettings();
        const version = GoogleAIProvider.isGeminiThinkingModel(modelDef)
            ? "v1alpha"
            : "v1beta";
        const url = `https://generativelanguage.googleapis.com/${version}/models/${modelDef.id}:generateContent?key=${this.apiKey}`;
...
function addProviderEx(displayName, defaultUniqueId) {
    ...
    addProvider(displayName, async (pluginRequest, abortSignal) => {
        try {
            return {
                success: true,
                content: await RequestHandler.handleRequest(pluginRequest, defaultUniqueId),
            };
        }
        catch (error) {
            let errorMessage;
