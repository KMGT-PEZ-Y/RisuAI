...
5. **이 설정으로 새 대전 시작**을 누릅니다. 행동 → 스킬 → **선택한 행동 실행** 순서로 진행합니다. 내/상대 장착 스킬 버튼은 고정 스펙 설명을 오버레이로 엽니다. 재클릭·다른 스킬 클릭·바깥 클릭·Escape를 지원하며, Enter/Space로도 열 수 있습니다.
6. 기본 연출은 종전 속도입니다. **빠른 진행**을 켜면 애니메이션 대기 없이 결과를 확인할 수 있습니다. 다운·그로기 등 강제 턴도 버튼으로 한 단계씩 진행합니다.

대전 설정으로 돌아가도 현재 경기는 유지됩니다. 캐릭터·덱 설정 변경은 새 경기에 적용되며, **진행 중인 경기로 돌아가기**로 이어갈 수 있습니다. 스킬 아이콘 켜기/끄기는 진행 중인 경기에도 반영됩니다. 켜면 양측 장착 스킬이 클릭 가능한 이미지로 표시되며 기존 설명 오버레이를 유지합니다. `/battle`은 설정 패널을 다시 엽니다.

행동 버튼 아래의 **판정 테이블**은 같은 모듈 안에서 27개 일반 판정과 3개 그로기 판정을 보여 줍니다. **전투 화면으로 돌아가기**로 복귀해도 선택과 연출 시계는 그대로 유지됩니다. 표의 HP/STA/BRK는 내 변화 / 상대 변화이며, 스킬 보정과 자원 상한·다운 적용 전의 기본값입니다.

2026-09-08 UI 개편: 가로 대치 초상화·하단 상태 바, 검정/빨강 팔레트, 고정 설명 칸 제거, 프레임 내부의 순차 연출을 적용했습니다. 최신 검증 및 환경 제한은 **UI_VALIDATION.md**를 참고하세요.

연출 보완: 둥근 주사위가 굴러간 뒤 실제 눈을 표시하며, 행동→판정→반응 이미지는 부드럽게 확대/축소·이동·페이드합니다. 스탠딩에는 체력별 박동, 다운/그로기에는 느린 박동을 적용했습니다. 이번 턴 스킬은 이름/상태가 한 버튼 안에 있고, 실행할 수 없으면 실행 버튼에 그 사유가 표시됩니다.

동일 이미지 반복 보완: 연속 단계의 에셋 경로가 같으면 한 구간으로 합쳐 유지하고, 이미지가 달라질 때만 pop-in/out을 적용합니다. 이전/복귀 초상화와 같은 이미지인 경우에도 재등장 효과 없이 이어집니다. 수정 직전 최신 배포본은 `BattleSim-RisuAI.before-deduplicated-transitions.charx`에 백업했습니다.
...
| 상대 정보 | 장착 스킬 버튼과 정적인 레벨별 스펙 설명. 현재 CD·잔여 사용 횟수·준비/예약 효과 상세는 출력하지 않음. 기존 HP/STA/BRK·다운·그로기 HUD는 유지 |
| 사진 | 기존 38종 + 공용 `skill_{스킬ID}.png`; 기존과 같은 `img src="{{raw::...}}"` |
| 기존 저장 | v1의 자원·다운·난수·전투 진행을 유지하고 양쪽 스킬만 빈 장착으로 초기화 |
| 진행 속도 | 기존 연출 유지 + 빠른 진행 선택 |

사진 제작·등록은 아직 하지 않았습니다. 사진이 없는 상태에서도 대전은 가능합니다. 스킬 아이콘은 기본 꺼짐이며 에셋을 추가한 뒤 켜면 장착 스킬 버튼, 스킬 설명, 주사위 아래의 사용 스킬 표시에 출력됩니다. 캐릭터·레벨과 무관하게 같은 아이콘을 공유하며, 스킬 사용 시에도 기존 행동·결과·반응 21장으로 초상화를 연출합니다. 전체 파일명·용도·비율은 **ASSETS.md**에 있습니다.
...
| `src/presentation.lua` | 공유 판정 테이블의 표시용 변환, 초상화·프레임 연출 |
| `src/battle.css` | 반응형 전투 화면, 팝오버·판정 화면·애니메이션 타임라인 |
| `src/skills.css` | 설정 화면과 공용 스킬 버튼 스타일 |
...
테스트는 `qa.py`, `qa_wasmoon.mjs`, `qa_browser.mjs`(실제 구현은 `qa_ui.mjs`), `qa_regression.mjs`에 있으며 개발용 의존성은 배포 파일에 들어가지 않습니다. 회귀 검사는 Git HEAD의 배포 Lua와 작업 파일을 대조합니다. CSS는 기존처럼 CHARX의 `backgroundHTML`에 넣습니다.

프레임에 맞춰 표시하는 이미지(초상화, 행동 연출, 스킬 썸네일·상세·주사위 아이콘)는 `margin-top:0`을 명시합니다. 호스트 채팅 스타일의 이미지 위쪽 여백이 레이아웃에 유입되는 것을 막기 위한 규칙이며, 같은 용도의 이미지를 추가할 때도 적용합니다.
