...
- 문서 버전: 1.0
- 확정일: 2026-09-01
- 구현 상태: Python Phase A-D 완료. 2026-09-04 M/U/H 30종과 공통 상태/할인/선택 제한 확장 구현. 상세는 `docs/MUH_ACTIVE_SKILLS.md`.
- 기준 문서: `docs/ROUND_TURN_BATTLE_POC_RULEBOOK.md`
...
완료 기록: 2026-09-01 `src/skill_schema.py`와 `tests/test_skill_schema.py`에 구현했다. 스키마 전용 테스트 13개가 통과한다.
...
완료 기록: 2026-09-01 `src/battle_sim.py`와 `tests/test_skill_runtime.py`에 구현했다. Phase B 테스트 14개가 통과하며, 100,000경기 무스킬 기준선은 Player 49.419%, Enemy 50.032%, 평균 34.4389턴으로 변경 전과 정확히 일치한다.
...
완료 기록: 2026-09-01 `src/battle_sim.py`와 `tests/test_skill_effects.py`에 구현했다. 즉시 적용 방식의 필수 timing, condition, priority와 여섯 효과 카테고리를 12개 Phase C 테스트로 검증했다. 이후 Player만 최대 4개 스킬을 장착·사용하고 무스킬 Enemy와 대전하는 UI 테스트베드와 테스트 8개를 추가했다. `on_status_apply` 실제 호출은 Phase D에서 연결했다.
...
완료 기록: 2026-09-01 `src/battle_sim.py`, `src/skill_schema.py`, `tests/test_phase_d_effects.py`에 구현했다. 상태 payload와 예약 효과를 기존 timing·priority 파이프라인에 연결하고 `on_status_apply`, 다음 자기 턴 활성화, refresh·replace, 정화 불가, 신규 상태의 후행 저장, `on_trigger`·`on_successful_apply`·`never` 소비를 Phase D 테스트 10개로 검증했다. 이어 Player 전용 테스트베드에 Phase D 예제 6종과 상태·예약 표시를 연결했다. 전체 자동 테스트 86개가 통과하며, 100,000경기 무스킬 기준선도 Player 49.419%, Enemy 50.032%, 더블 KO 0.549%, 평균 34.4389턴으로 정확히 유지된다.

### Phase E — 스킬 세트와 AI

- [x] 여섯 효과 카테고리를 검증하는 C/D 예제 스킬 세트 보존
- [x] 3레벨 예제 스킬과 무스킬·저레벨·고레벨·다중 스킬 장착 데이터 보존
- [x] 쉬움 적 3종의 플레이 경험·스킬 설계 기조 8항·사용 일정 합의
- [x] 쉬움 적 스킬 5종, 예정형 선택기와 동일 장착 랜덤 Player 구현
- [x] 새 선택기의 블라인드 선택·시드 재현·기본 행동·난수 보존 검증
- [x] M/U/H 액티브 스킬 30종 1레벨과 양측 수동/랜덤 테스트베드 구현
- [x] NG+ 공용 평가·3턴 탐색, 자유 장착·판단 충실도 UI, 실제 실행기 일치·블라인드 선택 검증 (`docs/NG_PLUS_AI.md`)
- [ ] 적별 난이도 보정과 다양한 컨셉 덱 성능, 새 스킬 상위 레벨 수치 설계·검증

2026-09-03 롤백: 이전 스킬 선택 알고리즘, 네 공통 선택기, 다섯 성향 보정,
NPC/Player 프리셋과 관련 설정·전용 테스트·설계 문서를 제거했다.
기존 구조나 파라미터를 새 설계의 전제로 삼지 않는다. Phase A-D 입력 검증과 효과 실행은 변경하지 않는다.

이후 같은 날 쉬움 적에 한정한 새 설계를 구현했다. `docs/EASY_ENEMY_SKILLS.md`가 이번 구현의 수치·순번·실패 계약 기준이다. Player도 새 5종을 수동 선택할 수 있다. 전체 106개 테스트 통과 및 적별 1,000경기 비교 완료; 수치는 플레이테스트 초안이다.
...
- [x] 쉬움 적 실험 JSON에 스키마 버전·스킬 데이터·양측 장착·일정 기록
