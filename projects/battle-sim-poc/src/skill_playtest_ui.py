...
class SkillPlaytestApp:
    ...
    def _build_ui(self) -> None:
        ...
        self._bind_skill_tooltip(self.active_skill_combo, self.active_skill_var)
        self.root.bind("<ButtonPress>", self._hide_skill_tooltip, add="+")
        self.root.bind("<Escape>", self._hide_skill_tooltip, add="+")
        self.root.bind("<Configure>", self._on_tooltip_root_configure, add="+")
    ...
    def _show_skill_details(self, index: int) -> None:
        ...
        self._replace_text(self.skill_details, self._skill_detail_text(skill_id))

    def _bind_skill_tooltip(self, combo: ttk.Combobox, variable: tk.StringVar) -> None:
        for sequence in ("<<ComboboxSelected>>", "<Enter>", "<FocusIn>"):
            combo.bind(sequence, self._show_turn_skill_tooltip, add="+")
        for sequence in ("<Leave>", "<FocusOut>", "<Destroy>"):
            combo.bind(sequence, self._hide_skill_tooltip, add="+")
        variable.trace_add("write", lambda *_: self._hide_skill_tooltip())

    def _hide_skill_tooltip(self, _event=None) -> None:
        if self._skill_tooltip is not None:
            self._skill_tooltip.destroy()
            self._skill_tooltip = None

    def _on_tooltip_root_configure(self, event) -> None:
        if event.widget is self.root:
            self._hide_skill_tooltip()

    def _show_turn_skill_tooltip(self, event) -> None:
        self._hide_skill_tooltip()
        combo = event.widget
        skill_id = self.skill_id_by_label.get(combo.get())
        if skill_id is None:
            return
        window = self._skill_tooltip = tk.Toplevel(self.root)
        window.withdraw()
        window.overrideredirect(True)
        window.attributes("-topmost", True)
        tk.Label(
            window, text=self._skill_detail_text(skill_id), justify="left",
            anchor="w", wraplength=520, padx=10, pady=8,
            background="#fffbe6", foreground="#202020", relief="solid",
            borderwidth=1, font=("Malgun Gothic", 9),
        ).pack()
        window.update_idletasks()
        width, height = window.winfo_reqwidth(), window.winfo_reqheight()
        x = max(0, min(combo.winfo_rootx(), combo.winfo_screenwidth() - width))
        y = combo.winfo_rooty() + combo.winfo_height() + 4
        if y + height > combo.winfo_screenheight():
            y = max(0, combo.winfo_rooty() - height - 4)
        window.geometry(f"+{x}+{y}")
        window.deiconify()

    def _skill_detail_text(self, skill_id: str) -> str:
