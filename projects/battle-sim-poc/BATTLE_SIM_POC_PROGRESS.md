...
- 최종 갱신일: 2026-09-03
- 현재 단계: Phase A-D 유지 + 쉬움 적 3종의 새 스킬·예정형 선택기·동일 장착 랜덤 Player·수동 UI 완료
- 다음 우선순위: 쉬움 적의 패턴 학습·빈틈을 직접 플레이로 검증. 보통 이상 적은 별도 설계
...
현재 자동 테스트는 총 106개이며 모두 통과한다. 기존 무스킬 전투 23개, 스키마 14개, Phase B 14개, Phase C 12개, Player 전용 테스트베드 13개, Phase D 10개에 쉬움 적 스킬·선택 계약·랜덤 비교 20개를 추가했다.
...
- 패시브 스킬
- 보통 이상 NPC의 스킬·선택 운용과 새 스킬의 상위 레벨 수치 (쉬움 3종·랜덤 Player 선택은 구현)
...
2. 확인한 요구만 조정하고 보통 이상 적의 스킬·운용을 별도 설계한다. 폐기한 선택기와 파라미터는 재사용을 전제하지 않는다.
